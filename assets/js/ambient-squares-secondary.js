(() => {
  ...
  const hosts = Array.from(document.querySelectorAll('[data-ambient-squares-secondary]'));
  ...
  supportStyle.textContent = `
    [data-ambient-squares-secondary] {
      position: relative;
      isolation: isolate;
      overflow: hidden;
    }

    [data-ambient-squares-secondary="about"] > .container {
      position: relative;
      z-index: 2;
    }

    .contact-box[data-ambient-squares-secondary="contact"] > .contact-center {
      position: relative;
      z-index: 3;
    }
  `;
  ...
  const ensureAudioContext = async () => {
    ...
    if (!audioContext) {
      ...
      audioMaster.gain.value = isSoundEnabled() ? .56 : 0;
  ...
  const syncAudioMaster = () => {
    if (!audioContext || !audioMaster) return;
    audioMaster.gain.cancelScheduledValues(audioContext.currentTime);
    audioMaster.gain.setTargetAtTime(isSoundEnabled() ? .56 : 0, audioContext.currentTime, .016);
  };

  const heroSoundToggle = document.querySelector('.ambient-sound-toggle');
  if (heroSoundToggle) {
    heroSoundToggle.addEventListener('click', () => {
      if (isSoundEnabled()) ensureAudioContext().then(syncAudioMaster);
      else syncAudioMaster();
    });

    const observer = new MutationObserver(syncAudioMaster);
    observer.observe(heroSoundToggle, { attributes: true, attributeFilter: ['aria-pressed'] });
  }
  ...
  /* Same selected HERO sound preset: Cartoon Confetti. */
  const playConfettiBurst = (tier, size) => {
    if (!isSoundEnabled() || !AudioContextCtor) return;

    ensureAudioContext().then((context) => {
      if (!context || !isSoundEnabled() || context.state !== 'running' || !audioMaster) return;
      syncAudioMaster();
      ...
      popGain.gain.exponentialRampToValueAtTime(profile.popGain, now + .004);
      popGain.gain.exponentialRampToValueAtTime(profile.popGain * .23, now + .055);
      ...
      for (let i = 0; i < profile.count; i += 1) {
        ...
        const peak = profile.sparkleGain * random(.52, 1);
  ...
  const desktopZones = [
    { x: 3, y: 100 }, { x: 13, y: 101 }, { x: 23, y: 99 }, { x: 38, y: 101 },
    { x: 54, y: 99 }, { x: 70, y: 101 }, { x: 84, y: 99 }, { x: 97, y: 100 }
  ];

  const mobileZones = [
    { x: 5, y: 97.5 }, { x: 18, y: 98.5 }, { x: 32, y: 97.8 }, { x: 50, y: 98.8 },
    { x: 68, y: 97.8 }, { x: 82, y: 98.5 }, { x: 95, y: 97.5 }
  ];
  ...
  const createEngine = (host) => {
    ...
    const riseFor = (size) => {
      ...
      const layerHeight = layer.getBoundingClientRect().height || host.getBoundingClientRect().height || 720;
    ...
    const corridorIsSafe = (zone, size, tier) => {
      const width = layer.getBoundingClientRect().width || host.getBoundingClientRect().width || 1200;
    ...
    const pickZone = (tier, size, avoidIndex = -1) => {
      ...
      const candidates = indexed.filter(({ zone, index }) =>
        !occupiedZones.has(index) && index !== avoidIndex && tierAllowsZone(zone, tier) && corridorIsSafe(zone, size, tier)
      );
      if (candidates.length) return choose(candidates);

      const safeAnyTier = indexed.filter(({ zone, index }) =>
        !occupiedZones.has(index) && index !== avoidIndex && corridorIsSafe(zone, size, tier)
      );
    ...
    const createCloud = ({ x, y }, color, size) => {
      ...
      const spread = size >= 170 ? size * .82 : size >= 90 ? size * .72 : Math.max(26, size * .62);
    ...
    const createDust = (origin, color, size) => {
      ...
      const spread = size >= 170 ? size * 1.08 : size >= 90 ? size * .92 : Math.max(54, size * .76);
      const baseDuration = reducedMotion.matches
        ? 340
        : size >= 170 ? random(1780, 2450) : size >= 90 ? random(1450, 2050) : random(1080, 1680);
    ...
    function spawnSquare(tier, avoidZone = -1) {
      ...
      window.setTimeout(() => {
        if (!square.isConnected || square.dataset.state === 'bursting') return;
    ...
    document.addEventListener('pointermove', (event) => {
      ...
      if (event.target instanceof Element && event.target.closest('.ambient-sound-toggle')) {
        pointerSeen = false;
    ...
    document.addEventListener('pointerdown', (event) => {
      if (finePointer.matches || (event.target instanceof Element && event.target.closest('.ambient-sound-toggle'))) return;
